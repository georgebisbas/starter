...
int padfunc(struct dataobj *restrict vp_vec, const int x_M, const int y_M, const int abc_x_l_ltkn, const int abc_x_r_rtkn, const int abc_y_l_ltkn, const int abc_y_r_rtkn, struct profiler * timers, const int x_m, const int y_m)
{
  float (*restrict vp)[vp_vec->size[1]] __attribute__ ((aligned (64))) = (float (*)[vp_vec->size[1]]) vp_vec->data;
  #pragma omp target enter data map(to: vp[0:vp_vec->size[0]][0:vp_vec->size[1]])
  ...
  #pragma omp target update from(vp[0:vp_vec->size[0]][0:vp_vec->size[1]])
  #pragma omp target exit data map(release: vp[0:vp_vec->size[0]][0:vp_vec->size[1]])
  return 0;
}
